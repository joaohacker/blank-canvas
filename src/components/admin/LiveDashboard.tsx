...
import { Activity, Users, Zap, Clock, RefreshCw, Wallet, AlertTriangle, CheckCircle, XCircle, CheckCheck } from "lucide-react";
...
export function LiveDashboard() {
  ...
  const handleForceStatus = async (gen: Generation, newStatus: "completed" | "cancelled") => {
    setForcing(gen.id);
    try {
      const { error } = await supabase
        ...
        .update({
          ...
          error_message: newStatus === "cancelled" ? "Cancelado manualmente pelo admin" : null,
        })
        .eq("id", gen.id);
      ...
      setGenerations((prev) =>
        prev.map((g) =>
          g.id === gen.id
            ? { ...g, status: newStatus, settled_at: new Date().toISOString(), error_message: newStatus === "cancelled" ? "Cancelado manualmente pelo admin" : null }
            : g
        )
      );
      toast.success(`Geração marcada como ${newStatus === "completed" ? "concluída" : "cancelada"}`);
    } catch (err: any) {
                      ...
                      {isActive && (
                        <>
                          <Button
                            ...
                            disabled={forcing === gen.id}
                            onClick={() => handleForceStatus(gen, "completed")}
                          ...
                          <Button
                            ...
                            disabled={forcing === gen.id}
                            onClick={() => handleForceStatus(gen, "cancelled")}
